...
sample_type_list = np.unique(df['Job::Sample Type From Sample Table'])  # savinng this list to write to a file later
...
print("The types of Oxalic I's in this wheel are {} ".format(np.unique(primary_standards['Description from Sample'])))
print("The average RTS of the Primary Standards in this wheel is {} \u00B1 {}".format(round(prim_std_average, rounding_decimal),round(prim_std_1sigma, rounding_decimal)))
print("The average OX-1 13C values in this wheel is {} \u00B1 {}".format(round(prim_std_13average, rounding_decimal), round(prim_std_13_1sigma, rounding_decimal)))
print()
...
if len(result) > 0:
    ...
    print("No OX-1's 13C values deviate from IRMS 13C values more than {}\u2030 ".format(C13_threshold))
    print()
# </editor-fold>
...
for i in range(0, len(refs['Sample Type From Sample Table'])):  # run through the list of "Sample Type from Sample Table"
    ref_row = refs.iloc[i]  # grab the first row of blank correction reference
