...
"""
May 2025; 
JCT requested that we add data from 2024 to 2026 (the present). Previously we had just been working with Albert's data 
but now that it's taken so long, so much new data has accumulated that it would be a mistake to exclude it. 
It should not be too burdensome to add this new data...
To expedite this process, I'm going to only work with the R numbers of secondaries and blanks. 
In the past work, I started by quality checking all data (including unknowns). This is not a viable option right now as we're way too delayed for this work. 
Quality checking just the secondaries and blanks for the added time (2024-2026) is more approachable
...
Before V6, we had been using this file; 
...
But JCT requests that we add data from 2024 to 2026, or essentially add data to the present. 
The previous file's max TP was 88370, max TW was 3518.

I've thrown out Albert's old export and restarted with a fresh one, cleaned up in the initial 0.py files
...
======= Read in the data cleaned up from files 0.py! ==========
...
df = pd.read_excel(r'C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\data\SECONDARIESONLY_FLASKLABELS_FIRILABLES.xlsx')
...
def flagging_status_check(comment):
    ...
    with pd.ExcelWriter(rf'C:\Users\clewis\IdeaProjects\GNS/xcams\Data_Quality_Version_6\output\Data_quality_paper_1_V6_output/{comment}.xlsx', engine="openpyxl") as writer:
...
for_removal = pd.read_excel(r"C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\data\Manually_checked_data.xlsx")
...
jct = pd.read_excel(r"C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\data\Final_RLIMS_IMPORT_toshare.xlsx")
...
df.loc[(df['F_corrected_normed_error'].isna()), 'Comment'] = 'Removed because absence of FM data'
df.loc[(df['F_corrected_normed_error'].isna()), 'Keep_Remove'] = 'Remove'
...
rs = rs[rs >= 10]
rs = pd.DataFrame(rs)
rs.to_excel(r'C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\output/Data_quality_paper_1_V6_output/Rs_more_than_10.xlsx')
rs = pd.read_excel(r'C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\output/Data_quality_paper_1_V6_output/Rs_more_than_10.xlsx')
rs = rs['Job::R']
...
outdir = r"C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\output/Data_quality_paper_1_V6_output/plotly_manual_flag"
...
for i in range(0, len(rs)):
    ...
    plt.savefig((rf'C:\Users\clewis\IdeaProjects\GNS\xcams\Data_Quality_Version_6\output/Data_quality_paper_1_V6_output/3_sigma_flag/{rs[i]}.png'))
...
extra_drops = [69746, # 40430/2, perhaps a small, larger errorbars? Internal Comment says >3sigma offset for both secondaries on this wheel. So I think its oK to remove.
                      # JCT 11/16 All secondaries were very low in this wheel, which was during period of XCAMS issues.  Have increased the wtw err to 0.4 % to reflect that problem.
               69747, # beacuse the wheel above was labelled as bad, I'll remove the other secondary from here. 
               89607, # notes indicate it had a problematic tune! 
               91478, # SCREW PUMP TESTS 19/09/2025 T.F. - Spike air test run on dreamliner for screw pump tests.; outside 3 sigma result
               86160, # 40430/2, also perhaps a small? Much larger errors; Internal Comment: All results in this wheel have a higher-than-normal wheel to wheel error to reflect higher than normal scatter in the primary standards
               90570, # 41347/12, very high residual. Definitely should go
               89656, # the next 4 are air sampes that were run in a test wheel, not using Flask Oxalics, which may lead to some confusion, why no flask oX?
               89657, 
               89658,
               89243,
               ]
df.loc[(df['TP'].isin(extra_drops)), 'Keep_Remove'] = 'Remove'
df.loc[(df['TP'].isin(extra_drops)), 'Comment'] = 'Extra drops in V6 beacuse new outliers are found with new data added, see comments in script'

flagging_status_check('13_manual_drop')
