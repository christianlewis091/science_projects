...
# drop some columns and rename new columns to match the old columns
df_new = df_new[['Job::R','Quality Flag','F_corrected_normed','F_corrected_normed_error','D14C', 'D14C_Error','Comment']]
...
new_sites = df_total.loc[df_total['Comment'] == 'TW3516']
...
for i in range(0, len(sites)):
    ...
    plt.savefig(
        f'C:/Users/clewis/IdeaProjects/GNS/soar_tree_rings/output_OPEN_ACCESS/Tree_Ring_Second_Check/{sites[i]}_errorbar.png',
...
merged.to_excel(r'C:\Users\clewis\IdeaProjects\GNS\soar_tree_rings\data\test2.xlsx')
#
# find_dups = df_total.loc[(df_total['Comment'] == 'TW3516') | (df_total['Comment'] == 'Old Runs')]
# duplicates = find_dups[find_dups['R number'].duplicated(keep=False)]
# # duplicates.to_excel(r'C:\Users\clewis\IdeaProjects\GNS\soar_tree_rings\data\dups.xlsx')
# # there are 5 instances of actual duplicates from the current test:
# list1 = ['41019/8','41019/30','41019/34','41019/36']
# dups = df_total.loc[df_total['R number'].isin(list1)].reset_index(drop=True)
# # dups.to_excel(r'C:\Users\clewis\IdeaProjects\GNS\soar_tree_rings\data\dups.xlsx')
# sites = np.unique(dups['Site'])
# for i in range(0, len(sites)):
#
#     # loop into the first site
#     this_site = dups.loc[dups['Site'] == sites[i]]
#     old = this_site.loc[this_site['Comment'] == 'Old Runs']
#     new = this_site.loc[this_site['Comment'] == 'TW3516']
#
#     # how many unique tree cores are in this site?
...
#         old = old.loc[old['TX_CY'] == cores[j]]
#         # used to do errorbar but they're too small to se anyway compared to the data
#         plt.errorbar(old['Ring year'], old['∆14C'], yerr=old['∆14Cerr']*1, label=f'{cores[j]}, OLD RUNS', color='red', marker=markers[j], linestyle='')
#
#     # how many unique tree cores are in this site?
#     cores = np.unique(new['TX_CY'])
#     for j in range(0, len(cores)):
#         new = new.loc[new['TX_CY'] == cores[j]]
#         # used to do errorbar but they're too small to se anyway compared to the data
#         plt.errorbar(new['Ring year'], new['∆14C'], yerr=new['∆14Cerr']*1, label=f'{cores[j]}, TW3516', color='blue', marker=markers[j], linestyle='')
#
#     plt.plot(harm_xs, harm_ys, label='SH Atmosphere \u0394$^1$$^4$CO$_2$ (\u2030)', color='black', alpha=0.2)
#     plt.legend()
#     plt.xlabel('Date', fontsize=14)
...
#     plt.ylabel('\u0394$^1$$^4$CO$_2$ (\u2030)', fontsize=14)  # label the y axis
#     plt.savefig(
#         f'C:/Users/clewis/IdeaProjects/GNS/soar_tree_rings/output_OPEN_ACCESS/Tree_Ring_Second_Check/{sites[i]}_DUPS_ONLY.png',
#         dpi=300, bbox_inches="tight")
#     plt.close()
