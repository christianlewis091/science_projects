...
def blank_corr_101322(input_name, date_bound_input, num_list):
    ...
    for i in range(len(num_list)):
        num = int(num_list[
                      i])  # needs to take the input from the user and change it from string to int before the loc function
...
#
#     # I'm skipping the code block related to the OX-1's because it's largely unnecessary for the groundwork of this
#     # next iteration.
#
    ...
    stds_hist = stds_hist.loc[(stds_hist['wtgraph'] > 0.3)]  # Drop everything that is smaller than 0.3 mg.
    stds_hist = stds_hist.loc[(stds_hist['Ratio to standard'] < 0.1)]  # Drop all blanks that are clearly WAY too high
    stds_hist = stds_hist.loc[(stds_hist[
                                   'Category In Calculation'] != 'Background Test')]  # Drop all background tests # TODO check this line with the team
...
#
    ...
    for i in range(0, len(refs)):     # Loop through the reference list
        ref_row = refs.iloc[i]
        reference_pretreat = ref_row['Pre-treatment Type']
        flag = 0
        for k in range(0, len(df)):   # Loop through the samples
            curr_samp_row = df.iloc[k]
            samp_pretreat = df['Pre-treatment Type']  # TODO have Valerie create this output column to data file

            if reference_pretreat == samp_pretreat:
                # print('I found a match: {}'.format(sample_type))
                mcc_arr2.append(ref_row['MCC'])
                # x = (0.45*ref_row['MCC'])
                mcc_1sigma2.append((0.45 * ref_row['MCC']))
        ...
        if flag == 0:  # if a match was not found, check if it was used for primary standards, or tuning, then fill with -999
                not_found.append(samp_pretreat)
                mcc_arr2.append('')
                mcc_1sigma2.append('')
                r_arr.append('')
                pretreatment.append('')
    ...
    # Write data to excel
    with pd.ExcelWriter(r'I:/C14Data/C14_blank_corrections_dev/TW{}_iteration2test.xlsx'.format(input_name)) as writer:
        refs.to_excel(writer, sheet_name="Current MCCs", index=False)
        stds_dataframe.to_excel(writer, sheet_name="Current MCC Details", index=False)
        df.to_excel(writer, sheet_name="TW{}_results".format(input_name), index=False)
